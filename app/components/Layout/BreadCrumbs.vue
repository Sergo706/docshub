...
const bread = computed(() => {
    ...
    const mapped = mapContentNavigation(crumbs, { deep: 0 })
        .map(item => ({ ...item, label: truncate(item.label ?? '', 25) }));

    if (!isBlog.value) {
      return mapped.map(item => item.label === 'Docs' ? { ...item, to: '/docs/getting-started' } : item)
