...
Checks if there is an active anomaly associated with the `session` and if its resolvable. Reject if the anomaly is not resolvable, and requires a full login, or return a response if it is. 
...
::note
This middleware reads the `anomaliesCache` to get its data and determine the response.
::
...
#### Route handlers
`authenticationRoutes` -> `tokenRotationRoutes` -> `magicLinks` -> `bffAccessRoute` -> `/operational/config`
