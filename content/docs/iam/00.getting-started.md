...
2. Creates all required auth MySQL tables (`users`, `sessions`, `rate_limiters`, etc.).
...
For the fastest path, the module exports `bootstrapApp`, which wires up the full middleware chain (Helmet, IP validation, HMAC, cookie parser, Bot Detector, all routes, error handling) in the correct order and returns a ready-to-use Express app:
...
10. `express.json()`
11. `cookie-parser`
12. Bot Detector `ApiResponse` middleware
13. Authentication routes
14. Token rotation routes
15. Magic link routes
16. BFF access route
17. Operational config route
18. Bot Detector warm-up
19. 404 handler
