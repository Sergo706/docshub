...
| `GET` | `/secret/accesstoken/metadata` | `requireAccessToken`, `requireRefreshToken`, `getFingerPrint`, `protectRoute`, `cookieOnly` | `getAccessTokenPayload` | Token TTL and rotation hint |
...
### `cookieOnly` (metadata route only)
...
`cookieOnly` enforces a strict no-body, no-query, no-content-type contract on the metadata endpoint.
