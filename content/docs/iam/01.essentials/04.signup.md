...
See [Routes](/docs/iam/routes) for how to mount the authentication router on your Express application.
...
A second union limiter is keyed on a composite of `ip + email`. The burst window is 1 point per second (blocks for 30 minutes) and the slow window is 3 points per 24 hours (blocks for 24 hours). This prevents an attacker from distributing signup attempts for the same email across multiple IPs.
