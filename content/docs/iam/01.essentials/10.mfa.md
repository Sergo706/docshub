...
`sendTempMfaLink` generates a unique JTI (random UUID + 128 bytes of `crypto.randomBytes`), a random challenge (128 bytes), and hashes the challenge with SHA-256. It then signs a temporary JWT with purpose `MAGIC_LINK_MFA_CHECKS` and a subject of `MAGIC_LINK_MFA_CHECKS_{visitor}`.
...
`POST /custom/mfa/:reason` initiates a custom MFA flow. The controller enforces IP restrictions (the request must originate from `service.clientIp` or `service.proxy.ipToTrust`), applies four layers of rate limiting, and validates the request body with Zod.

```ts
// Request body
{
  "random": "...",   // 254-500 character random challenge string
  "reason": "payment"
}
```
...
| `/auth/update-email` | POST | Submits new email + password + OTP for email change |
| `/auth/reset-password` | POST | Initiates a password reset email |
