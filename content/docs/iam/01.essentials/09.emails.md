...
Sends an OTP email using the `OTP/index` template. Called internally by `sendTempMfaLink` and `generateCustomMfaFlow`.

```ts
import { mfaEmail } from '@riavzon/auth'
...
Sends a notification email using the `nottifications/index` template. Called internally by `sendTempPasswordResetLink` when a user requests a password reset.

```ts
import { resetPasswordEmail } from '@riavzon/auth'
...
Sends a customizable notification email. Called by the email update controller to alert the user that their email address has changed, and available for any custom notification.

```ts
import { sendEmailNotification } from '@riavzon/auth'
