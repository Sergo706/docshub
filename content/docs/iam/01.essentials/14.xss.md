...
The function performs six sequential stages. Each stage builds on the previous one, and an attacker must bypass all of them for a payload to survive.

::steps{level="4"}
#### Unicode normalization

The input is normalized to [NFKC](https://unicode.org/reports/tr15/) form, which collapses visually similar Unicode characters into their canonical equivalents. This defeats attacks that use fullwidth characters (e.g. `\uFF1C` for `<`) or other Unicode homoglyphs to bypass pattern matching. Zero-width characters, soft hyphens, byte-order marks, and bidirectional override characters are stripped.
...
If the initial `decodeURIComponent` call throws (malformed percent-encoding such as `%ZZ`), the input is rejected immediately. Legitimate user input does not contain malformed URI sequences, so rejecting early avoids wasting cycles on broken data.
...
If any pattern matches, `htmlFound` is set to `true` in the results. This flag is used downstream by `validateSchema` to trigger IP banning.

#### sanitize-html pass

The string is passed through [sanitize-html](https://www.npmjs.com/package/sanitize-html) with a strict zero-tolerance configuration:

- **No tags allowed** (`allowedTags: []`)
- **No attributes allowed** (`allowedAttributes: {}`)
- **No iframes, no URL schemes, no protocol-relative URLs**
- **Non-text tags** (`script`, `style`, `noscript`, `iframe`, `svg`) are discarded entirely
- **Nesting limit** of 10 prevents deeply nested tag bombs

The `onOpenTag` callback records every tag the sanitizer encounters, populating the `results.tags` field with the tag name and attributes. If the string length decreases after sanitization (meaning content was stripped), `htmlFound` is set to `true` even if the pattern detection stage did not trigger.

#### Entity encoding

The final output is entity-encoded for safe rendering in any context. Every character that could start or close an HTML context is replaced with its entity equivalent:
...
The backtick and template literal escaping prevent injection into JavaScript template strings.
::
...
`validateSchema` ties the full pipeline together. It parses input against a Zod schema, and when any Zod issue starts with `'HTML found'` (produced by `makeSanitizedZodString`), it calls `handleXSS` to ban the client immediately.

```ts
import { validateSchema } from '@riavzon/auth'

const result = await validateSchema(commentSchema, req.body, req, log)
...
// Called automatically by validateSchema — you rarely need to call this directly
...
|---|---|---|
| `initPasswordReset` | `POST /auth/reset-password` | Prevents enumerating which email addresses have accounts |
...
The full pipeline works in four steps: define a schema with `makeSanitizedZodString`, validate with `validateSchema`, check the result, and use the sanitized data.

```ts
import { validateSchema, makeSanitizedZodString } from '@riavzon/auth'
import { z } from 'zod'
...
router.post('/profile', async (req, res) => {
  ...
  const result = await validateSchema(profileSchema, req.body, req, log)
