...
| Method | Path | Purpose | Middleware | Reference |
|---|---|---|---|---|
| POST | `/auth/user/refresh-session` | Rotate both access and refresh tokens | `requireRefreshToken`, `cookieOnly`, `getFingerPrint` | [Refresh Tokens](/docs/iam/essentials/refresh-tokens) |
| POST | `/auth/logout` | Revoke the current session | `requireRefreshToken`, `requireAccessToken`, `cookieOnly` | [Logout](/docs/iam/essentials/logout) |

The `cookieOnly` middleware enforces that these requests send only cookies: no JSON body, no query string, and no `Content-Type` header. This prevents CSRF attacks by ensuring the request cannot carry attacker-controlled data beyond the browser's automatic cookie attachment.
...
| POST | `/update/email` | Change the user's email address | |
| POST | `/auth/forgot-password` | Initiate a password reset | [Magic Links](/docs/iam/essentials/magic-links) |
| GET | `/auth/reset-password` | Verify a password reset link | [Magic Links](/docs/iam/essentials/magic-links) |
| POST | `/auth/reset-password` | Submit a new password | [Magic Links](/docs/iam/essentials/magic-links) |
...
| POST | `/update/email` | Access + Refresh + JWT | JSON (1 KB) | |
| POST | `/auth/forgot-password` | None | JSON (1 KB) | [Magic Links](/docs/iam/essentials/magic-links) |
| GET | `/auth/reset-password` | Magic link token | None | [Magic Links](/docs/iam/essentials/magic-links) |
| POST | `/auth/reset-password` | Magic link token | JSON (1 KB) | [Magic Links](/docs/iam/essentials/magic-links) |
