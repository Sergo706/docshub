...
2. Issues a CSRF cookie via `generateCsrfCookie` so the subsequent POST has a valid token.
3. Checks that the `canary_id` cookie and `token` query parameter are present.
4. Proxies the request to the IAM service for link signature verification.
