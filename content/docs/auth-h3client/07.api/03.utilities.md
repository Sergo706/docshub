...
const { server, enableFireWallBans, telegram } = getConfiguration()
```
...
Posts a message to the configured Telegram bot. Only executes when `telegram.enableTelegramLogger` is `true`.
