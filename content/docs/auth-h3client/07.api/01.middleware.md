...
It combines `defineByteLimiterHandler(..., 2000, 'POST')` and
`defineAuthenticatedEventPostHandlers(...)`, validates `event.context.params`
against the supported actions, and then proxies the request to the IAM API
token management endpoints.

Supported actions are `new-token`, `revoke`, `metadata`, `rotate`,
`ip-restriction-update`, and `privilege-update`. The wrapper rejects
