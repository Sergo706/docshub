...
The module ships four route registrars that mount authentication, OAuth, magic link, and bounce routes on an H3 router or app. Register them explicitly during startup by calling each registrar with your router or app instance, as shown in the [H3 and Nitro setup](/docs/auth-h3client/guides/h3-nitro) guide. When using the Nuxt module, all registrars are called automatically during plugin setup.
...
#### `generateCsrfCookie`
Mints a fresh CSRF cookie for the page that will be served.
...
Returns the current session state by proxying to the IAM `/secret/data` endpoint. Registered automatically by the Nuxt module at the path configured in `authStatusUrl`.
...
| `GET` | `/auth/users/authStatus` | Module (auto) | Check session state |
